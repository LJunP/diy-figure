<template>
  <div class="refill-page">
    ...
    <div v-loading="loading" class="refill-container">
      <!-- 补购说明 -->
      <div class="refill-info glass">
        <h2 class="page-title">补购 - 订单 #{{ orderId }}</h2>
        <div class="info-items">
          <div class="info-pill">
            <el-icon color="#667eea"><Coin /></el-icon>
            <span>补购定价:(套餐总价 ÷ 中签数) × 1.3</span>
          </div>
          <div class="info-pill">
            <el-icon color="#43e97b"><Clock /></el-icon>
            <span>补购窗口:主订单发货后 60 天内有效</span>
          </div>
          <div class="info-pill">
            <el-icon color="#f5576c"><Flag /></el-icon>
      ...
      <div v-if="refillableCanvases.length > 0">
        <h3 class="section-title">未中签角色 ({{ refillableCanvases.length }})</h3>
        <div class="refill-grid">
          <div v-for="canvas in refillableCanvases" :key="canvas.canvasId" class="refill-card glass" :class="{ expired: canvas.expired }">
            <div class="rc-visual">
              <el-image v-if="canvas.firstConceptImage" :src="canvas.firstConceptImage" fit="cover" class="rc-img" />
              <div v-else class="rc-img no-img"><el-icon :size="32" color="#333"><Picture /></el-icon></div>
              <div class="rc-status" :class="canvas.expired ? 'expired-badge' : 'available-badge'">
                {{ canvas.expired ? '已过期' : '可补购' }}
              </div>
            </div>
            <div class="rc-body">
              ...
              <el-button v-if="!canvas.expired" type="primary" round size="small" :loading="refilling === canvas.canvasId" @click="handleRefill(canvas)" class="rc-btn">
                发起补购
              </el-button>
            </div>
          </div>
      ...
      <!-- 补购订单列表 -->
      <div v-if="refillOrders.length > 0" class="refill-orders glass">
...
.refill-info { padding: 28px 32px; border-radius: 16px; margin-bottom: 28px; }
.page-title { font-size: 22px; font-weight: 700; color: #fff; margin-bottom: 16px; }
.info-items { display: flex; flex-direction: column; gap: 8px; }
.info-pill { display: flex; align-items: center; gap: 8px; color: #aaa; font-size: 14px; }
...
.refill-card { border-radius: 14px; overflow: hidden; transition: all 0.3s; }
.refill-card:hover { transform: translateY(-4px); border-color: rgba(102,126,234,0.3); }
...
.rc-status { position: absolute; top: 8px; left: 8px; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; }
.available-badge { background: rgba(67,233,123,0.8); color: #fff; }
.expired-badge { background: rgba(255,255,255,0.2); color: #ccc; }

.rc-body { padding: 16px; text-align: center; }
.rc-name { font-size: 15px; font-weight: 600; color: #fff; margin-bottom: 8px; }
.rc-price { font-size: 20px; font-weight: 800; color: #f5576c; margin-bottom: 8px; }
.rc-deadline { display: flex; align-items: center; justify-content: center; gap: 4px; font-size: 12px; color: #666; margin-bottom: 12px; }
.rc-btn { width: 100%; }

.empty { display: flex; flex-direction: column; align-items: center; padding: 60px; border-radius: 16px; gap: 16px; }
.empty p { color: #666; }

.refill-orders { padding: 24px 32px; border-radius: 16px; margin-top: 28px; }
