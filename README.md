...
>
> 核心页面和多数业务服务已有初版实现；前端生产构建已通过；运营接口已有服务端角色校验，取消费用按真实支付事实计算并有单元测试覆盖；已有 Flyway 迁移脚本、生产配置与容器化部署文件。支付仍为模拟流程（入口已隔离但未接真实渠道），真实退款、并发一致性、备份恢复演练与供应链参数仍存在缺口。当前版本不适合直接开放真实付费业务。
>
> 状态核对日期：**2026-09-14**（工作区，HEAD `b34e884` + 未提交改动）。本文区分“已有代码”“部分或模拟实现”“尚未开发”和“尚未验证”，不把接口存在视为产品验收通过。
...
| 人工报价 | 录入价格与预计交期；用户接受或拒绝；接受时校验报价已填写且大于 0；**报价弹窗内展示系列档位、设计数量、中签数量与设计稿**（中签角色高亮），作为定价依据 | 交期为空时是否允许接受，规则待确认 | 初版已实现 |
| 随机抽选 | 按档位抽取角色，保存中签和未中签结果 | 并发与重复请求下的结果唯一性未验证 | 初版已实现 |
| 定金与尾款 | 支付单、支付记录、模拟成功、订单状态联动与画布锁定；模拟入口有环境隔离与支付单归属校验 | 微信和支付宝真实下单、验签、金额核对、对账均未完成 | 模拟实现 |
...
| 能力 | 当前情况 |
| --- | --- |
| 数据层 | JPA 实体与 Repository 已有；Flyway 迁移脚本 V1（12 张表）/ V2（乐观锁）/ V3（3D 任务）/ V4（列类型修正）/ V5（画布角色名称）；生产 profile 为 `ddl-auto=validate`；开发库首次启用时自动基线 |
| 后端公共能力 | 统一响应、异常处理、参数校验、JWT 拦截器、管理员角色拦截器、健康检查、订单状态日志、站内消息与邮件通知 |
| 自动化测试 | 后端集成+单元回归 + 前端 Vitest（金额与 SSE 解析）；CI 都跑 |
| 构建验证 | 后端编译与测试通过；前端 Vitest 12 个用例 + 生产构建通过（`npm run build` 约 4s，仅剩 chunk 体积告警） |
| 部署交付 | 多阶段 `Dockerfile`、`docker-compose.yml`、`.env.example`、`application-prod.yml`、GitHub Actions CI（后端测试 + 前端构建）；仍缺经过验收的回滚流程 |
| 开源协作 | 仍无 `LICENSE`、独立贡献指南和安全报告流程；已补充根 `.gitignore` 防止 `.env` 误提交 |
...
**FACT（已验证事实）：**以下为 2026-09-14、工作区（HEAD `b34e884` + 未提交改动）的本地检查结果，不是持续集成徽章或发布认证。

| 检查 | 结果 | 能证明什么 |
| --- | --- | --- |
| `mvn -o -DskipTests compile` | **通过**，编译 104 个 Java 源文件 | 本地依赖条件下后端可编译；未执行测试 |
...
| 阻塞真实交易 | 支付仍为模拟模式；真实下单、验签、金额核对、对账未完成 | [PaymentService](backend/src/main/java/com/diyfigure/payment/PaymentService.java)、[PaymentController](backend/src/main/java/com/diyfigure/payment/PaymentController.java) | **部分修复**：模拟入口已加环境隔离（`payment.mock-enabled` + `prod` 硬拒绝）与支付单归属校验；真实支付仍未接入 |
...
| `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET` | 上传文件 | 默认占位配置跳过客户端初始化，参考图改落本地磁盘并返回可访问 URL（不再返回固定占位图） |
...
| `DIY_SCHEDULING_ENABLED` | 定时任务总开关 | 默认 `true`；集成测试置 `false`，避免调度线程在重建表时干扰用例 |
| 支付渠道 | 定金与尾款 | 固定模拟实现；没有配置商户密钥即可开启真实支付的能力 |

演示分支依据特定占位字符串判断，尚未统一成可靠的模式开关；空字符串或无效密钥不保证自动降级。OSS 未配置或转存失败时，生成图片可能保留外部原始 URL，不保证长期可访问。参考图上传在 OSS 未配置时会落本地磁盘，属于本机可访问而非公网可访问，且 AI 对话传给 GPT-4o 的图片 URL 需要模型侧能拉取，本机地址不满足这一点。占位图片也依赖外部网络，因此演示模式不等于完全离线。
...
当前构建可通过（`npm run build` 已实测成功）。项目已接入 Vitest（`frontend/src/utils/` 下小工具函数有单元测试，CI 顺带跑）。仍缺 lint（暂未启用 ESLint）。`preview` 用于查看本地构建产物；生产环境还需要静态托管、SPA 路由回退、`/api` 反向代理与 SSE 超时和缓冲配置，不能直接套用 Vite 开发代理。产物里 `index` 与 `three.module` 两个 chunk 超过 500 kB，需要时再按路由做代码分割。
...
- [x] 明确定稿、审核、锁定、补购规则，补齐拒绝后重提流程。已完成：终审拒绝后重提、拒绝报价后重开、拒绝理由回显，以及返工闭环最后一环"提交终审"；剩余：未中签画布的锁定规则确认。
- [x] 为金额计算、状态机规则与取消违约金建立必要测试。已完成：金额拆分、状态机红线、取消违约金三档、乐观锁、邮件降级等 30 个单元用例。
- [x] 用隔离数据库跑通主订单、返工、取消、补购与过期分支。（2026-09-13：独立 schema `diy_figure_it`，含支付幂等与越权检查；2026-09-14 扩充到 49 个集成用例，新增画布生命周期与文件上传降级通道）
...
- [x] 数据库迁移脚本：接入 Flyway，`V1__init_schema.sql` 已通过"空库启动 + `ddl-auto=validate`"双验证。剩余：备份与恢复验证。
...
表结构由 Flyway 维护：空库首次启动会执行 `V1__init_schema.sql`，已存在的库会自动打基线（不会重复执行 V1，也不会动既有数据）。后续变更请新增 `V2__xxx.sql`，不要修改已执行过的版本文件。
