...
> ### 已停止开发（2026-09-21）· 内部 Alpha 归档
>
> 保留角色设计、画布、订单、审核与履约状态管理的 Web/API 实现、模拟闭环及历史测试材料。
> 支付与履约仍为模拟实现；AI/3D 等接口存在真实接入分支，不等于已完成真实服务、样品、供应链或商业交付验证。
> 不适合真实付费业务；本次归档未补业务功能或重新运行测试，不宣称生产可用。
> 本项目已退出作者的 LLM 推理基础设施学习路线；后续学习与新项目不依赖本仓库的代码、数据或运行环境。
> 下文路线、开发说明及未完成项仅保留为历史记录，不再作为继续投入或资产迁移的任务。

**从一个原创角色想法，到一套由自己参与设计的实体盲盒。**

> ### ⚠️ 内部 Alpha · 支付与履约为模拟实现 · 不适合真实业务
>
> 一句话：**代码闭环齐全，商业闭环没有。** 别拿它收钱。

DIY Figure 面向原创角色收藏、个性化礼物与定制手办场景，将 AI 对话设计、系列管理、人工终审、报价抽选、分期付款与生产履约组织在同一套系统中。用户表达创意并确认设计，平台运营负责审核、报价、厂家对接、质检与发货。

> 展开说明：软件侧可开发内容已收口并推送到 `main`；尚未进入真实付费与真实履约。
>
...
以下保留原阶段 0–4 的历史定义，已随停止开发决定冻结；不是当前待办、排期或交付承诺。
...
本仓库按内部 Alpha 归档，不再接受新的功能或修复工作。[已知问题](#已知问题)及尚未验证的真实交易边界继续保留。

原贡献方式与安全说明保留在 [CONTRIBUTING.md](CONTRIBUTING.md) 和 [SECURITY.md](SECURITY.md)，仅作历史参考。

请勿在公开材料中暴露密钥、个人地址、真实支付信息或数据库数据。
